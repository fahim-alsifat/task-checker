...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
    ...
    const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            const success = importChecklistFromJson(content);
            if (success) {
    ...
    return (
        ...
        `}
            >
                ...
                {/* Footer */}
                <div className="p-4 border-t border-[#252525] space-y-3">
                    {/* Export/Import buttons */}
                    <div className="flex gap-2">
                        <button
                            onClick={() => {
                                if (activeChecklistId) {
                                    exportChecklistAsJson(activeChecklistId);
                                } else {
                                    alert('Please select a checklist to export.');
                                }
                            }}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg transition-colors text-xs"
                            title="Export selected checklist"
                        >
                            <ExportIcon />
                            Export
                        </button>
                        <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg transition-colors text-xs"
                            title="Import checklist from JSON"
                        >
                            <ImportIcon />
                            Import
                        </button>
